    ...
    <form @submit.prevent="addSlider">
      ...
      <div class="form-group">
        ...
        <div v-if="form.image_path" class="row">
          <div class="col-6">
            <img :src="form.image_path" class="img-thumbnail" />
          </div>
        </div>
        <input
          type="file"
...
export default {
  ...
  methods: {
    addSlider() {
      this.$axios
        .$post('http://127.0.0.1:8000/api/sliders', {
          name: this.form.name,
          description: this.form.description,
          image_path: this.form.image_path,
        })
        .then((result) => {
          this.$toast.success('Thêm slide mới thành công!')
